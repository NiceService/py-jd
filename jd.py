...
class JD:
    ...
    def login(self):
        ...
        # logins = s.post('http://i.jd.com/user/info', headers=headers)
        # print(logins.text)
    ...
    def saveHtml(file_content):  
        ...
        f = open("jd/user_info_url.htm", "w+b")
...
if result.get('success'):
    ...
    print('抓取京东某个用户数据')
    user_info_url = "http://i.jd.com/user/info"
    # html = getHtml(user_info_url)
    logins = s.post(user_info_url, headers=headersd)
    jd.saveHtml(logins)
else:
    print('登录失败')
